# This is the Pipeline functions with looping for Prompt methods in zero-shot, few-shot and full-data cases
# copy of prompt_pipeline_loop.py, this file is designed to make it possible for run multiple files at the same time
import os
import time
...
experi_type_list = ['few_shot'] # 'zero_shot', 'full_data'
# For few-shot cases, we use these loops
num_example_list = [256] # [ 8, 16, 32, 64, 128, 256]
sampler_seed_list = [11, 300, 13]
for experi_type in experi_type_list:
    if experi_type == 'few_shot':
        for num_example_in_loop in num_example_list:
            for sampler_seed_in_loop in sampler_seed_list:
                # Here, we list all the parameters which define the experiment
                #--------------------------------------------------------------------------------------------------------------------------------------------------------------------
                'Parameters Group 1: Experiment_details'

                # Please define experiment type here: zero_shot, few_shot or full_data
                experi_type = experi_type
                set_seed(144)
                num_train_epochs = 3
                use_cuda = True
                ...
                if experi_type == 'zero_shot':
                    ...
                    num_example = num_example_in_loop # This is number of examples per label, total num of examples will be 2*num_example
                    sampler_seed = sampler_seed_in_loop # determine the random sample of data (11)
                ...
                model_trainable = True # whether the model is trainable, follows from previous model definition
                ...
                if model_template == 'manual':
                    ...
                    mytemplate = MixedTemplate(
                        ...
                        text = '{"placeholder": "text_a"} {"soft": "bad"} {"soft"} health condition means yes readmission, {"soft": "recovery"} {"soft"} means no readmission, predict readmission: {"mask"}',
                ...
                if model_verbalizer == 'manual':
                    auto_extraction = True # Whether we extract label words from tran.csv similar to what knowledgeVerbalizer did
                    if auto_extraction == True: # Special: No trainable parameter for prompt itself, but requires data
                ...
                if model_verbalizer == 'soft':
                    myverbalizer = utils.SoftVerbalizer(tokenizer, plm, num_classes=2).from_file('/home/s2174572/mlp/mlp1/verbalizer_calibration.txt')
                ...
                if model_verbalizer == 'knowledgeable':
                    ...
                    remove_current_label_words_address = True # whether remove current_label_words.txt file
                    num_extraction = 200 # num of labels to be extracted
                ...
                print(prompt_model)
                print('label_words')
                print(prompt_model.verbalizer.label_words)
                print('label_words_weights')
                print(prompt_model.verbalizer.label_words_weights)
                ...
                'verbalizer'
                prompt_lr_verbalizer1 = 3e-2
                prompt_lr_verbalizer2 = 6e-2
                warmup_step_verbalizer = 20

                eval_every_steps = 2 # default value 500, store best model every 500 steps
                ...
                # -- -- -- -- -- -- -- ---- -- -- -- -- -- -- ---- -- -- -- -- -- -- ---- -- -- -- -- -- -- ---- -- -- -- -- -- -- ---- -- -- -- -- -- -- ---- -- -- -- -- -- -- --
                    ...
                    if experi_type == 'few_shot':
                        ...
                        if sample_criterion == 'clinical_notes': # TODO: full_val_set and full_test_set cases
                            ...
                            print('We will randomly sample', num_example, 'examples per label')
                            print('Sampling process based on:', sample_criterion)
                            mimic_data_modified_dir = utils.Group_and_Sample(mimic_data_dir, num_examples_per_label = num_example, seed = sampler_seed, location = 'result_all_prompt/prompt_pipeline')
                            dataset = {}
                            # read data and get different splits
                    ...
                    """
                    After we get dataset, we can modify our verbalizer in same cases:
                    """
                    # = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = =  = = = = = = = = 
                    # Under few_shot/full_data cases
                    # NOTE: for few-shot/full_data + Knowledgeable Verbalizer case, the label_words are defined from each sampled train.csv
                    normal = True # normally, we get label words from train.csv, this is set to false only for some testing cases
                    if (model_verbalizer == 'knowledgeable' and normal == True) or (model_verbalizer == 'manual' and auto_extraction == True):
                        print("Automatically extracting label words from train.csv")
                        if experi_type == 'few_shot':
                            train_dir = f'{mimic_data_modified_dir}/train.csv'
                        elif experi_type == 'full_data':
                            train_dir = f'{mimic_data_dir}/train.csv'
                        ...
                        _ = utils.label_words_from_statistics(train_dir,num_extraction, current_label_words_address, list0, list1)
                        ...
                        # Some testing here:
                        # print('prompt_model verbalizers label words')
                        # print(prompt_model.verbalizer.label_words)
                        # print('prompt_model verbalizers label words weights')
                        # print(prompt_model.verbalizer.label_words_weights)
                ...
                # ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                    ...
                    def evaluate2(prompt_model, dataloader):
                        ...
                        # used for validation_dataloader & test_dataloader
                        prompt_model.eval()
                        allpreds = []
                        alllabels = []
                        IDs = []
                    ...
                    # - - - - - - - - - - -  - - - -  - - - -- - - - - - - - - - -  - - - -  - - - -- - - - - - - - - - -  - - - -  - - - -- - - - - - - - - - -  - - - -  - - - -- - - - - - - - - - -  - - - -  - - - -- - - - - - - - - - -  - - - -  - - - -- - - - - - - - - - -  - - - -  - - - -- - - - - - - - - - -  - - - -  - - - -
                    if model_trainable == False: 
                        '''for few_shot and full_data cases, there are also untrainable case'''
                        # In this case, there is no training step, but only inference step
                        _, df_for_summary = evaluate_plot(prompt_model, test_dataloader, location)
                        test_acc_, test_loss_, roc_auc_test_, prc_auc_test_, rp80_test_ = evaluate2(prompt_model, test_dataloader)

                    elif model_trainable == True:
                        '''for few_shot and full_data cases, there are also untrainable case'''
                        # If the model trainable, there possibily be three parts of model parameters, namely from plm, template and verbalizer
                    ...
                    if experi_type == 'full_data' and model_trainable == True:
                    ...
                    # Include lable_words and their weights
                    if model_verbalizer in ['knowledgeable','manual']:
                        print('prompt_model verbalizers label words')
                        label_words = prompt_model.verbalizer.label_words
                        print(label_words)
                        content_write += f"label_words{label_words}\n"

                    if model_verbalizer == 'knowledgeable':
                        print('prompt_model verbalizers label words weights')
                        label_words_weights = prompt_model.verbalizer.label_words_weights
                        print(label_words_weights)
                        content_write += f"label_words_weights{label_words_weights}\n"
                    ...
                    if (model_trainable == True) and (remove_final_model == True):
