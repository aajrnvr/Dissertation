# This is the Pipeline functions for test of Prompt methods in zero-shot, few-shot and full-data cases
import os
import time
...
if experi_type == 'zero_shot':
    ...
    num_example = 8 # This is number of examples per label, total num of examples will be 2*num_example
    sampler_seed = 13 # determine the random sample of data (11)
...
if model_verbalizer == 'knowledgeable':
    ...
    remove_current_label_words_address = False # whether remove current_label_words.txt file
...
'verbalizer'
prompt_lr_verbalizer1 = 3e-3
prompt_lr_verbalizer2 = 3e-2
warmup_step_verbalizer = 300

eval_every_steps = 500 # default value 500, store best model every 500 steps
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            for step, inputs in enumerate(train_dataloader):
                ...
                if actual_step % gradient_accumulation_steps == 0 and glb_step >0 and glb_step % eval_every_steps == 0:
                    ...
                    if val_acc >= best_val_acc and roc_auc_score > 0.9*best_val_roc and prc_auc_score > 0.9*best_val_prc:
                        ...
                        _, df_for_summary = evaluate_plot(prompt_model, test_dataloader, location)
                        test_acc, test_loss, roc_auc_test, prc_auc_test, rp80_test = evaluate2(prompt_model, test_dataloader) # TODO: waste computational resource here: need to optimize
    ...
    # Printing out label_words and label_words_weights
    print('prompt_model verbalizers label words')
